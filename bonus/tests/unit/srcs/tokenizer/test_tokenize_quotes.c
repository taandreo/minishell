...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/15 15:29:37 by ebezerra          #+#    #+#             */
/*   Updated: 2023/09/15 15:31:35 by ebezerra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	(void)state;
	char *input = ft_strdup("\"$VAR $?abc\"$VAR2abc");
	t_token_list *tokens = create_token_list();
	t_token_flags flags = init_token_flags(strlen(input));
	size_t pos = 0;
...
void test_handle_quotes_success(void **state)
{
	...
	flags = init_token_flags(strlen(input));
	pos = 0;
...
////	assert_int_equal(tokens->tail->token.type, TOKEN_ERROR);
	...
	char *str = strdup("\"$VAR2 $? * s test\"$VAR2abc");
	t_token_list *tokens = create_token_list();
	t_token_flags flags = init_token_flags(strlen(str));
	size_t pos = 0;
	setup_malloc_behavior(SUCCESS_MALLOC,1);
	char *quoted_string = handle_quotes(&str, &pos, &tokens, &flags);
	assert_null(quoted_string);
}
...
void test_extract_quoted_string(void **state)
{
	...
	pos = 0;
	flags = init_token_flags(input_len);
	flags.quote_type = '\"';
	flags.inside_quotes = true;
	flags.string = ft_strdup("");
...
void test_initialize_var_string(void **state)
{
	...
	t_token_flags flags = init_token_flags(len);
